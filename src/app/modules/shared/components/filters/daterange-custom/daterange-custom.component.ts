...
@Component({
  selector: 'app-daterange-custom',
  templateUrl: './daterange-custom.component.html',
  styleUrls: ['./daterange-custom.component.scss'],
})
export class DaterangeCustomComponent implements OnInit {
  @Input() urlDateRange: any;
  @Input() dateApplied = true;
  @Input() eventPage = false;
  @Input() genesisPage = false;
  @Input() importPage = false;
  ...
  ngOnInit(): void {
    ...
    if (this.urlDateRange) this.setAppliedDate(this.urlDateRange);
      ...
      if (this.eventPage) {
        this.timeQueryChanged(
          { label: 'Last 1 Hour', value: 'last_1_hour' },
          this.dateApplied
        );
      }
       else if (this.genesisPage) {
        this.timeQueryChanged(
    ...
    if (dateArray[0] && dateArray[1]) {
      ...
      if (diff <= 1439)
        diff = differenceInMinutes(
          this.add_minutes(dateArray[1], 1),
          dateArray[0]
        );
      else
        diff = differenceInMinutes(
          this.add_days(dateArray[1], 1),
          dateArray[0]
        );
