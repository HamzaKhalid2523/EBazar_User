...
import { differenceInMinutes, format, subDays, subMinutes } from 'date-fns';

@Component({
  selector: 'app-daterange-menu',
  templateUrl: './daterange-menu.component.html',
  styleUrls: ['./daterange-menu.component.scss']
})
export class DaterangeMenuComponent implements OnInit {

  @Input() urlDateRange: any;

  defaultRangeValue = "last_7_days";
  ...
  @Output() loadingEmitter = new EventEmitter<any>();
  ...
  ngOnInit(): void {
    if (this.urlDateRange) this.setAppliedDate(this.urlDateRange);
      ...
      this.timeQueryChanged({ label: "Last 7 Days", value: "last_7_days" }, true);
    }
  ...
  CustomRangeChanged(event: any) {
    ...
    switch (event) {
      case "last_15_minutes":
        ...
        this.setRangeStartEnd(date_response.date_start, date_response.date_end, "last_15_minutes");

        break;
      case "last_30_minutes":
        date_response = this.calculateDateRangebyMinutes(30, now);
        this.setRangeStartEnd(date_response.date_start, date_response.date_end, "last_30_minutes");

        break;
      case "last_1_hour":
        date_response = this.calculateDateRangebyMinutes(1 * 60, now);
        this.setRangeStartEnd(date_response.date_start, date_response.date_end, "last_1_hour");

        break;
      case "last_3_hours":
        date_response = this.calculateDateRangebyMinutes(3 * 60, now);
        this.setRangeStartEnd(date_response.date_start, date_response.date_end, "last_3_hours");

        break;
      case "last_6_hours":
        date_response = this.calculateDateRangebyMinutes(6 * 60, now);
        this.setRangeStartEnd(date_response.date_start, date_response.date_end, "last_6_hours");

        break;
      case "last_12_hours":
        const twelve_hours = this.calculateDateRangebyMinutes(12 * 60, now);
        this.setRangeStartEnd(twelve_hours.date_start, twelve_hours.date_end, "last_12_hours");

        break;
      case "last_24_hours":
        date_response = this.calculateDateRangebyMinutes(24 * 60, now);
        this.setRangeStartEnd(date_response.date_start, date_response.date_end, "last_24_hours");

        break;
      case "today":
        // const now = new Date();
        this.setRangeStartEnd(now, now, "today");

        break;
      case "last_3_days":
        const three_days = this.calculateDateRangebyDays(3);
        this.setRangeStartEnd(three_days.date_start, three_days.date_end, "last_3_days");

        break;
      case "last_7_days":
        const seven_days = this.calculateDateRangebyDays(7);
        this.setRangeStartEnd(seven_days.date_start, seven_days.date_end, "last_7_days");

        break;
      case "last_10_days":
        const ten_days = this.calculateDateRangebyDays(10);
        this.setRangeStartEnd(ten_days.date_start, ten_days.date_end, "last_10_days");
      ...
      case "this_month":
        ...
        this.setRangeStartEnd(firstDay, todayDate, "this_month");

        break;
      case "last_30_days":
        const thirty_days = this.calculateDateRangebyDays(30);
        this.setRangeStartEnd(thirty_days.date_start, thirty_days.date_end, "last_30_days");

        break;
      case "last_90_days":
        const ninety_days = this.calculateDateRangebyDays(90);
        this.setRangeStartEnd(ninety_days.date_start, ninety_days.date_end, "last_90_days");
  ...
  setRangeStartEnd(start: any, end: any, dateRangeLabel: any, till_now = true) {
